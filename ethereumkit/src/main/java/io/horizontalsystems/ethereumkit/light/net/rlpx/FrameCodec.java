 ...
 */
package io.horizontalsystems.ethereumkit.light.net.rlpx;
...
import static io.horizontalsystems.ethereumkit.light.util.RLP.decode2OneItem;
import static io.horizontalsystems.ethereumkit.light.util.RLP.rlpDecodeInt;
...
public class FrameCodec {
    ...
    public List<Frame> readFrames(InputStream inp) throws IOException {
        if (!isHeadRead) {
            byte[] headBuffer = new byte[32];
            try {
                inp.read(headBuffer);
            ...
            protocol = rlpDecodeInt(rlpList.get(0));
            contextId = -1;
            totalFrameSize = -1;
            if (rlpList.size() > 1) {
                contextId = rlpDecodeInt(rlpList.get(1));
                if (rlpList.size() > 2) {
